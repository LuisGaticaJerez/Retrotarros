# RETROTARROS — Pauta de episodio largo

*Nostalgia + Juegos + Música*

**Los SNES mas caros del mundo — Lo que el mercado paga**

*Generacion 4 · Super Nintendo · Top 10 + Holy Grail*

Documento de trabajo · Luis Balbrigame & Koko

---

## Proposito de esta pauta

Episodio ranking de PRECIOS, no de calidad. Muestra los 10 cartuchos SNES mas caros del mercado CIB (Complete In Box) segun PriceCharting / Heritage Auctions / eBay sales 2026. Cierra con un kicker fuera de concurso: el **Nintendo PowerFest '94 NFR** ($23.000), holy grail absoluto del coleccionismo SNES.

Es la **contraparte de `snes-top-mundial`**: alli vimos el ranking de calidad/critica, aca el ranking de plata. **Spoiler** que se anticipa en el cold open: SOLO 1 juego aparece en los dos rankings.

---

## Concepto del episodio

Formato presentacion visual con HTML del estudio (`studio/rankings/top-precios/snes-top-precios.html`):

- 16 slides navegables del 01 al 16.
- Portada con tag "TOP 10 · MAS CAROS · CIB MUNDIAL"
- Recap rapido del top mundial al inicio (slide 02) para anclar al espectador.
- 10 slides de ranking #10 → #1 con cartucho + box art + nombre + meta + bloque "POR QUE ES CARO" justificando rareza.
- Slide kicker Nintendo PowerFest '94 ($23.000, 2 copias en el mundo).
- Balance + cierre.

**Duracion objetivo:** 20-25 minutos.

---

## El Top 10 por precio · PriceCharting CIB USD mayo 2026

Solo cartuchos retail NTSC USA, condicion CIB. Variantes NFR/promo van en kicker aparte.

| # | Juego | CIB USD | Por que tan caro |
|---|-------|--------:|------------------|
| **1** | Earthbound (Big Box) | $3.500 | Caja gigante VHS-size + player's guide oficial con stickers scratch-and-sniff. Cult RPG + status legendario. |
| **2** | Wild Guns | $2.500 | Western steampunk + galeria de tiros estilo Cabal. Tiraje NTSC limitado. Remaster 2016 disparo el original. |
| **3** | Aero Fighters | $1.500 | Shoot 'em up port de Neo-Geo. Los shmups SNES son escasos de por si. Sin variantes, sin reediciones. |
| **4** | Chrono Trigger | $1.200 | El JRPG perfecto. Caja amarilla brillosa se rayaba facil — boxes en buen estado son raros. |
| **5** | Mega Man X3 | $1.100 | Ultimo de la trilogia X en SNES, lanzado tarde. En EEUU solo SNES (Japon tuvo SNES + PS1 + Saturn). |
| **6** | Pocky & Rocky 2 | $1.000 | Secuela cult shmup japones con miko anime. Tiraje NTSC minimo. |
| **7** | Final Fantasy III (FF VI) | $800 | El JRPG mas prestigioso del SNES. Tiraje NTSC limitado vs la fama mundial. Caja con manual original es escasa. |
| **8** | Super Mario RPG | $700 | El unico Mario RPG en era 16-bit. Ultima colab Nintendo+Square antes de PlayStation. Remake Switch 2023 disparo demanda original. |
| **9** | Harvest Moon | $500 | Late release SNES = tiraje bajo. Boom retroactivo del genero farming sim (Stardew, Story of Seasons). |
| **10** | Demon's Crest | $400 | Spinoff oscuro de Gargoyle's Quest. Capcom lo lanzo al final del ciclo con poca promocion. Redescubierto post-2010. |

**Lectura del ranking:**

- **Solo 1 juego cruza con el top mundial:** Earthbound (que es #1 aca + #8 mundial). El resto del top calidad NO esta en top precios.
- **Patron:** 8 de 10 son "late releases" (1994-1996), cuando todos miraban PS1 y los tirajes bajaron drasticamente.
- **Generos sub-representados que valen mucho aca:** shmups (Wild Guns, Aero Fighters, Pocky & Rocky 2) y RPGs late-cycle.

---

## El Kicker · Nintendo PowerFest '94 NFR

Cartucho NO For Resale. Fuera de concurso porque no es retail, es promo de torneo.

| Variante | Precio | Por que importa |
|----------|-------:|-----------------|
| **Nintendo PowerFest '94** | **$23.000** | Torneo oficial Nintendo of America. Cartucho contiene mini-versiones de Super Mario Kart + Mario All-Stars + Ken Griffey Jr. Nintendo fabrico 33 unidades, desarmo 31 para repuestos. **Solo 2 sobreviven en el mundo.** Ultima venta documentada 2026. |

**Esto no es mercado, es museo.** El cartucho mas caro de SNES jamas vendido en una subasta documentada.

---

## Estructura del episodio (20-25 min)

### Bloque 1 · Cold open (0:00 – 0:30)

Plano cerrado de un cartucho random de la coleccion. Luis:

> *"Si te dijera que un cartucho SNES vale veintitres mil dolares, ¿me crees? Y no es Zelda. No es Mario. Es uno que solo dos personas tienen en el mundo. Pero antes de llegar a ese, vamos por los diez mas caros de mercado."*

Cut al logo Retrotarros.

### Bloque 2 · Setup (0:30 – 1:30)

- Luis aclara: ranking de **precio**, no calidad.
- Comparativa rapida con el top mundial: "solo uno aparece en los dos." Suspenso.
- Criterio: PriceCharting CIB NTSC USA mayo 2026.
- Koko: "ya sabes, voy a llorar por lo que no tengo."

### Bloque 3 · Recap top mundial (1:30 – 2:00)

Slide 02: muestra los 10 mundial para que el espectador no se pierda.
Luis: "esos son los buenos. Ahora vamos por los caros."

### Bloque 4 · Recorrido #10 → #1 (2:00 – 19:00)

17 min para 10 items = ~1:40 por item.

**Estructura por slide:**
- Posicion grande + cartucho fisico (Luis lo levanta).
- Box art + meta (consola/año/editor/precio).
- Bloque "POR QUE ES CARO": Luis lee + Koko reacciona.
- 15-20 seg por item base, 30-40 seg para los del top 3 (Aero Fighters, Wild Guns, Earthbound).

**Items donde NO ahorrar tiempo (top 3 + spoiler-cross):**
- **#1 Earthbound** — destacar la big box gigante + player's guide.
- **#2 Wild Guns** — anecdota del remaster 2016.
- **#7 FF VI** — momento "este SI es del top mundial tambien."

### Bloque 5 · Kicker PowerFest '94 (19:00 – 22:00)

Slide 12 (HOLY GRAIL).

- Luis cuenta la historia: torneo Nintendo 1994, 33 cartuchos, 31 desarmados.
- "Solo dos personas en el mundo tienen esto."
- Ultima venta 2026: $23.000.
- Cita: "Esto ya no es mercado, es museo."

### Bloque 6 · Balance + cierre (22:00 – 25:00)

- "El top 10 calidad tiene Mario, Zelda, Chrono. El top 10 precio tiene Earthbound, Wild Guns, Aero Fighters."
- "Solo Earthbound coincide. Eso te dice mucho."
- Cierre: "Cuentanos cual de estos tienes. O cual te gustaria tener. Sigue suscrito."

---

## Reglas de ejecucion en vivo

### Ritmo

- **Cada item max 1:40**. Si entran a divagar en un item del medio, Luis corta.
- **Los del top 3** pueden estirarse a 2 minutos cada uno.
- **El kicker PowerFest** tiene 3 minutos completos — es el clip que define el episodio.

### Lo que NO se dice

- No decir "este vale mas porque es mejor". Es **precio**, no calidad.
- No mencionar precios en CLP (los precios son globales USD por consistencia).
- No comparar con N64 o PS1 mas que como passing.

### Lo que SI se dice

- "Este lo tenemos / Este no lo tenemos" (autoridad de coleccionista).
- "Si lo encuentras suelto vale X, CIB vale Y" (diferencia loose vs CIB).
- Bromas sobre el "te lo pude haber comprado en 2010 por 50 lucas".

---

## Material visual necesario

### Para grabar (antes del episodio)

- [ ] 10 close-ups de cartuchos del top (idealmente los que Koko tenga fisicos).
- [ ] Foto/render Earthbound Big Box completa abierta (para slide #1).
- [ ] Foto del PowerFest '94 (si existe en archivos publicos).
- [ ] Tabla comparativa Top Mundial vs Top Precios (para el balance final).

### MP3s pre-cargados de TarroBot

Ya generados en `studio/pautas/audio/snes-top-precios/`:
- snes-top-precios-1.mp3 a snes-top-precios-10.mp3 (10 MP3s)

---

## Estado de la pauta

| Item | Estado |
|------|--------|
| HTML estudio (`studio/rankings/top-precios/snes-top-precios.html`) | ✓ Cerrado para grabar (16 slides) |
| Pauta JSON TarroBot (`studio/pautas/snes-top-precios.tarrobot.json`) | ✓ Con 10 MP3s pre-generados |
| Capturas (`studio/captures/snes-top-precios/`) | ✓ 16 PNGs |
| Pauta MD (este archivo) | ✓ |
| Discusion MD (`docs/discusiones/discusion-snes-top-precios.md`) | ✓ |
| Descripcion YouTube (`studio/exports/snes-top-precios-publicacion.txt`) | ✓ |
| Material visual (Earthbound Big Box, PowerFest) | ☐ Pendiente |

---

## Notas finales

- Episodio "fuerte" en datos. Maneja precios y rareza, mas que opiniones.
- **La cruz con top mundial es contenido narrativo clave** — el espectador que vio el otro episodio engancha aca por la diferencia.
- Reutilizable: el formato "top precios" replica facil para N64 (ya hay `docs/pautas/pauta-n64-top-precios.md`).
- El kicker PowerFest puede convertirse en Short propio: "El SNES mas caro del mundo en 30 segundos."

---

**Ultima actualizacion:** 2026-05-24
**Slug:** `snes-top-precios`
**HTML asociado:** `studio/rankings/top-precios/snes-top-precios.html`
**Discusion:** `docs/discusiones/discusion-snes-top-precios.md`
