# Pauta — PS1 Top Precios

> Episodio: Los discos de PS1 que hoy valen una fortuna (retail CIB + rarezas + santo grial)
> HTML: `studio/rankings/top-precios/ps1-top-precios.html`
> Formato: ranking hibrido por precio CIB · 19 slides

## Gancho / Cold open

Nada de esto es Final Fantasy VII ni Metal Gear Solid. Lo caro en PS1 son los JRPG y juegos de nicho japones que vendieron poco en Occidente. Hoy: del piso al techo del top 10 retail, mas rarezas y un santo grial que se vendio en subasta por 55 mil dolares.

## Formato (regla del canal)

- RETAIL CIB (#10 → #1): juegos vendidos en tiendas de Estados Unidos, ordenados por valor CIB.
- RAREZAS / NO-RETAIL: 3 variantes de empaque/edicion que no se vendieron en retail normal -Syphon Filter 3 [911 Edition], Mortal Kombat 3 [Jewel Case], NFL Blitz 2000 [Greatest Hits].
- SANTO GRIAL: Tekken [Long Box] -precio de una copia sellada y graduada, no del cartucho comun.
- Precios reales PriceCharting (consultado septiembre 2026). Valores fluctuan, nunca consejo de inversion.

## Retail CIB (#10 → #1)

| # | Juego | CIB | Por que |
|---|---|---|---|
| 10 | Suikoden II | ~$177 | Konami nunca reimprimio pese a la demanda creciente |
| 9 | Tales of Destiny II | ~$200 | Lanzamiento tardio de la consola (2002), confusion de nombre con la saga |
| 8 | Valkyrie Profile | ~$309 | Dialogo masivo, traduccion compleja, tirada limitada |
| 7 | JoJo's Bizarre Adventure | ~$312 | Conversion CPS3, arte exclusivo de Hirohiko Araki |
| 6 | Klonoa: Door to Phantomile | ~$409 | Vendio bien en Japon, distribucion occidental limitada |
| 5 | Clock Tower II | ~$458 | Recepcion tibia en su momento, hoy nicho de culto horror |
| 4 | Persona 2: Eternal Punishment | ~$490 | Unica localizacion oficial en ingles, demanda post-Persona 5 |
| 3 | Revelations: Persona | ~$508 | Localizacion muy alterada, nunca salio en Europa hasta PSP 2010 |
| 2 | Tail Concerto | ~$525 | Derechos de publicacion pasaron entre estudios, tirada retrasada |
| 1 | The Misadventures of Tron Bonne | ~$653 | Ventas tan bajas que Capcom nunca repitio el spin-off |

## Rarezas / No-Retail

- **Syphon Filter 3 [911 Edition]** (~$1.000+): portada original con edificio en llamas y bandera americana, retirada tras el 11-S. Nunca vendida oficialmente con esa portada.
- **Mortal Kombat 3 [Jewel Case]** (sin precio establecido): solo 3-4 copias confirmadas en jewel case cuadrado -la inmensa mayoria vino en long box.
- **NFL Blitz 2000 [Greatest Hits]** (~$3.600 sellado, ~$62 suelto): nunca debio tener edicion Greatest Hits, salio igual en tirada minima.

## Santo Grial

- **Tekken [Long Box]** (record USD 55.200): copia sellada y graduada Wata 9.8 A++, vendida en Heritage Auctions marzo 2024. El disco suelto comun vale USD 15-30, el CIB normal ronda los USD 90. ACLARAR SIEMPRE en camara que es precio de copia sellada/graduada, no del cartucho comun.

## Puntos polemicos (para comentarios)

- Persona 2 y Revelations: Persona subieron de precio retroactivamente por el boom de Persona 5 (2016) -dato de mercado real, no solo rareza original.
- La diferencia entre el CIB normal de Tekken Long Box (~$90) y el record de subasta graduado ($55.200) es el ejemplo mas extremo de todo el canal del fenomeno WATA/coleccionismo graduado.
- Mortal Kombat 3 Jewel Case no tiene precio de mercado establecido -aclarar que es la unica entrada del top sin numero fijo.

## Veredicto (slide 18)

Lo raro gana, no lo famoso. Ningun juego de este top aparecio en el Top Mundial. La regla se repite consola tras consola: tirada corta + publisher chico + localizacion limitada = precio alto. PS1 no es la excepcion.

## Cierre / CTA (slide 19)

¿Sabian de The Misadventures of Tron Bonne antes de este video? ¿Tienen algun disco de esta lista guardado? Comenten.

## Notas de produccion

- Box arts: 14/14 reales, verificadas visualmente. 10 de Wikipedia REST summary + LaunchBox Games Database para las que Wikipedia trajo version incorrecta (Tales of Destiny II traia la portada japonesa "Tales of Eternia"; JoJo's Bizarre Adventure traia un flyer de arcade de "JoJo's Venture", nombre distinto al del juego de PS1).
- Rarezas y grial requirieron busqueda extra fuera de Wikipedia/LaunchBox: Syphon Filter 3 911 Edition (blog especializado pricecharting.com), NFL Blitz 2000 Greatest Hits (game-rave.com, recortada del spread front+back+disc), Tekken Long Box (psxdatacenter.com, scan especifico del formato long box distinto del jewel case estandar).
- Notas teleprompter: tecla N o boton (no-capture). Precio en pantalla igual para retail/rarezas/grial (misma pastilla amarilla), salvo Mortal Kombat 3 Jewel Case que no lleva precio por no tener venta de referencia.
