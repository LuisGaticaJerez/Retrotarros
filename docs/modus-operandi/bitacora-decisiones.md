...
## 2026-08-06 — Capa de notas en `top_deck.py` + fixes de precios en Atari 2600 Top Precios

- **Bug real y sistémico: `top_deck.py` nunca implementó la capa de notas de lectura (teleprompter, tecla N)**, pese a que el CLAUDE.md la marca como obligatoria para todo HTML de ranking del formato nuevo desde 2026-06-15 y cita `studio/rankings/top-mundial/psvita-top-mundial.html` como patrón de referencia. Ese archivo SÍ tiene notas porque se armó a mano antes de que existiera el generador — pero `top_deck.py` jamás copió esa parte del patrón, así que **todos** los episodios generados con él desde entonces (SNES, N64, NES, Dreamcast, Saturn, Mega Drive, Master System, PS Vita top-precios, y los dos de Atari 2600) quedaron sin notas. Luis lo detectó al revisar Atari 2600 puntualmente. **Fix:** se portó el patrón de `resena_deck.py` (que sí lo tenía) a `top_deck.py` — helper `_notas()`, CSS de la capa, botón/tecla N inyectados por texto sobre el `foot` clonado del BASE (que tampoco lo trae). Quedó disponible para cualquier episodio nuevo; los anteriores (SNES, N64, etc.) siguen sin notas hasta que se regeneren con contenido nuevo — no se tocaron en este fix porque no fue pedido.
- **Notas reales escritas para los 2 episodios de Atari 2600** (14 bloques en top-mundial, 20 en top-precios): dato extra por slide + cue de presentador, dando MÁS información de la que aparece en pantalla (regla 2026-06-15). Top-mundial usa un hilo narrativo (3 de los 10 juegos son Activision, fundada en 1979 por programadores de Atari que renunciaron por falta de crédito) que conecta Kaboom!, River Raid y Pitfall! y remata en el cierre.
- **Bug de datos: Pepsi Invaders no mostraba precio en pantalla** (sin `USD` en su `meta`, `_price_of()` no encontraba nada que mostrar) y **el bloque de RAREZAS no estaba ordenado por valor**. Investigado con WebSearch: una copia de Pepsi Invaders se remató en eBay en **USD 1.825 en 2005** (fuente: Engadget, cruzado con el registro histórico del hobby) — se usó ese dato real para completar el precio. Rarezas reordenadas ascendente por valor verificado: Pepsi Invaders (1.825) → Gauntlet (~5.000) → Red Sea Crossing (~13.800) → Gamma Attack (500.000, pedido no venta real, cierre del bloque).
- **Dato descartado por no poder verificarse:** se iba a agregar que Quadrun requería el controlador "Video Touch Pad" — no se encontró respaldo real. Se reemplazó por un dato SÍ verificado en Wikipedia (fue el primer juego del 2600 con síntesis de voz, tirada de ~10.000 unidades por pruebas negativas contra Ms. Pac-Man). **Lección repetida:** verificar cada afirmación histórica antes de meterla en las notas, con la misma vara que el resto del guion — las notas no son "informales", también llevan la regla de respaldo verificable.

## 2026-08-08 — Bug real de Ñ en las 17 reseñas (14 drivers afectados)

- **Bug real y extendido: falta de Ñ en el texto on-screen de la mayoría de las reseñas.** Luis lo detectó revisando el canal ("aun tienen voceo argentino y problemas con la ñ"). Se auditó cada uno de los 14 drivers de reseña (`gen_resenas_batch.py` + 13 `gen_resena_*.py`) con un script de barrido por patrones (`disen`, `anio/ano/anos`, `companer`, `pequen`, `acompan`, `redisen`, etc.) — **13 de 14 tenían el bug** (`altered_beast` fue el único limpio). Palabras afectadas: "diseno/disena/disenado/disenio" → "diseño/diseña/diseñado/diseñó", "anio/ano/anos" → "año/años", "pequeno" → "pequeño", "companero" → "compañero", "acompana" → "acompaña", "redisenaron/redisenio" → "rediseñaron/rediseñó". Se corrigieron ~45 instancias en total repartidas en 13 archivos.
- **El voseo, en cambio, resultó estar limpio:** se corrió un segundo barrido con patrones de conjugación voseante (tenés, sabés, querés, jugás, vos, che, etc.) sobre los mismos 14 drivers y sobre `resena_deck.py` — cero coincidencias reales (solo falsos positivos tipo "banda" matcheando "anda"). El trabajo de corrección de voseo de la sesión 2026-07-29 sí quedó bien aplicado; el problema que Luis notó era específicamente la Ñ.
- **Causa probable:** al escribir el contenido de varias reseñas (sobre todo las 10 agregadas el 2026-07-29: Kirby x2, SMW, SFII, Sonic, ALttP, Mega Man 2, Golden Axe, EarthBound, Chrono Trigger) el caracter Ñ se perdió de forma sistemática en la escritura, algo que no había pasado en las 4 primeras del batch original salvo por los mismos campos de fecha ("anio/ano"). **Lección:** al escribir contenido para reseñas nuevas, revisar explícitamente la Ñ antes de cerrar — no asumir que "sin tildes" significa "sin caracteres especiales", la Ñ SIEMPRE debe ir correcta en el HTML on-screen (regla ya existente en `top_deck.py`: "la ñ si se permite").
- Los 17 HTML de reseña se regeneraron desde sus drivers corregidos y las 136 capturas (17×8) se regeneraron también. Verificado con un script de barrido final: 0 coincidencias de palabras sin Ñ en los 17 HTML de salida.
